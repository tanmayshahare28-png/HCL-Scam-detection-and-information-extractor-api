...
def generate_basic_response_for_scam_type(message: str, conversation_context: List = []) -> str:
    """
    ...
    if category != "unknown" and score > 0:
        ...
        # Use hash of message and conversation length to get varied responses
        idx = (hash(message) + len(conversation_context)) % len(responses)
    ...
    idx = (hash(message) + len(conversation_context)) % len(default_responses)
...
def generate_agent_response(text: str, session_id: str, scam_detected: bool,
    ...
    if not scam_detected:
        ...
        idx = min(msg_count - 1, len(normal_responses) - 1)
    ...
    try:
        ...
        # Create a prompt for Ollama based on the detected scam type and conversation context
        ...
        prompt = f"""You are a person who has received a suspicious message. The message appears to be a {scam_type_description} scam. The message says: '{text}'. {context_str}Respond as if you are a real person who is suspicious but wants to understand more about the situation. Be cautious but engage enough to extract more information from the scammer. Keep your response brief and natural. Do not acknowledge that you are an AI or bot."""
        
        # Call Ollama to generate a response
        ollama_response = call_ollama_api(prompt, OLLAMA_MODEL)
        ...
        if ollama_response and len(ollama_response.strip()) > 5:
            ...
            return generate_basic_response_for_scam_type(text, conversation_history)
...
def home():
    return jsonify({
        "message": "Honeypot API - Expanded Scam Detection System with Ollama",
        "version": "3.0.0",
        "status": "running",
...
if __name__ == "__main__":
    print("=" * 60)
    print("[ROCKET] HONEYPOT API - OLLAMA ENHANCED SCAM DETECTION SYSTEM")
    print("=" * 60)
    ...
    print(f"[MODEL] Ollama model: {OLLAMA_MODEL}")
    ...
    print("=" * 60)
    print("[LIBRARY] Expanded scam detection with 12 categories:")
