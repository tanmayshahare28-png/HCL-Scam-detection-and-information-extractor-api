...
def check_url_on_spotthescam(url: str) -> Dict:
    ...
    """
    try:
        # SpotTheScam.in API endpoint for URL checking
        api_url = "https://spotthescam.in/api/check"
        
        # Prepare the request
        headers = {
            "Content-Type": "application/json",
            "User-Agent": "Honeypot-API/1.0"
        }
        ...
        # Make the request to spotthescam.in
        response = requests.post(
            api_url,
            json=payload,
            headers=headers,
        ...
        if response.status_code == 200:
            ...
            # If the specific API doesn't exist, we'll simulate checking by making a HEAD request
            # to see if the site is known for malicious activity
            head_response = requests.head(url, timeout=5, allow_redirects=True)
            # In a real implementation, we would use the actual spotthescam.in API
            return {
                "url": url,
...
def home():
    return jsonify({
        "message": "AI-Powered Multi-Agent Honeypot System with Ollama Integration and URL Checking",
        "version": "7.0.0",
        "status": "running",
        "mode": API_VALIDATION_MODE,
        "timestamp": datetime.now().isoformat(),
        "agent_personalities_available": len(SCAM_CATEGORIES) + 1,  # +1 for normal
        "features": ["Ollama Integration", "SpotTheScam.in URL Checking", "Multi-Agent System", "Intelligence Extraction"]
    })
...
def health():
    return jsonify({
        ...
        "features": ["Ollama Integration", "SpotTheScam.in URL Checking"]
    })
...
def validate_api():
    ...
    return jsonify({
        "status": "success",
        "message": "Multi-Agent API with Ollama Integration and URL Checking is ready for evaluation",
        ...
        "features": ["Ollama Integration", "SpotTheScam.in URL Checking", "Multi-Agent System"],
        "timestamp": datetime.now().isoformat()
    })
...
if __name__ == "__main__":
    print("=" * 60)
    print("[ROCKET] ENHANCED HONEYPOT - ADVANCED AI SYSTEM WITH OLLAMA PRIORITY AND URL CHECKING")
    print("=" * 60)
    ...
    print("[URL CHECK] This version checks URLs on spotthescam.in for malicious indicators")
    print("=" * 60)
