...
# ========== EXPANDED SCAM CATEGORIES WITH AGENT PERSONALITIES ==========
SCAM_CATEGORIES = {
    "bank_fraud": {
        "patterns": [
        ...
        "agent_personality": "Financial Security Agent",
        "agent_trait": "Cautious and security-conscious, asks for verification details",
        "responses": [
    ...
    "upi_fraud": {
        ...
        "agent_personality": "Payment Security Agent",
        "agent_trait": "Tech-savvy and cautious about digital payments",
        "responses": [
    ...
    "investment_scam": {
        ...
        "agent_personality": "Investment Advisor Agent",
        "agent_trait": "Financially savvy, asks about risks and returns",
        "responses": [
    ...
    "lottery_scam": {
        ...
        "agent_personality": "Skeptic Agent",
        "agent_trait": "Doubtful of unexpected windfalls, asks for proof",
        "responses": [
    ...
    "tech_support_scam": {
        ...
        "agent_personality": "IT Security Agent",
        "agent_trait": "Technical and security-focused, verifies credentials",
        "responses": [
    ...
    "insurance_fraud": {
        ...
        "agent_personality": "Policy Holder Agent",
        "agent_trait": "Knows their policy details, verifies claims",
        "responses": [
    ...
    "tax_fraud": {
        ...
        "agent_personality": "Tax Compliance Agent",
        "agent_trait": "Knowledgeable about tax procedures, verifies authenticity",
        "responses": [
    ...
    "loan_fraud": {
        ...
        "agent_personality": "Credit Conscious Agent",
        "agent_trait": "Aware of lending practices, cautious about terms",
        "responses": [
    ...
    "phishing": {
        ...
        "agent_personality": "Cyber Security Agent",
        "agent_trait": "Security-focused, avoids suspicious links",
        "responses": [
    ...
    "romance_scam": {
        ...
        "agent_personality": "Relationship Skeptic Agent",
        "agent_trait": "Emotionally aware but financially cautious",
        "responses": [
    ...
    "job_fraud": {
        ...
        "agent_personality": "Career Conscious Agent",
        "agent_trait": "Professional, verifies job legitimacy",
        "responses": [
    ...
    "charity_fraud": {
        ...
        "agent_personality": "Donation Verifier Agent",
        "agent_trait": "Generous but verifies charity authenticity",
        "responses": [
...
def get_agent_info(category: str) -> Dict:
    ...
    if category in SCAM_CATEGORIES:
        return {
            "name": SCAM_CATEGORIES[category]["agent_personality"],
            "personality": SCAM_CATEGORIES[category]["agent_trait"],
            "responses": SCAM_CATEGORIES[category]["responses"]
        }
    else:
        return {
...
def call_ollama_api(prompt: str, model: str = "gemma3:4b") -> str:
    """
    Call Ollama API to generate a response based on the scam type and conversation context
    """
    try:
        ...
        # Clean up the response to remove any potential artifacts
        lines = response.split('\n')
        cleaned_lines = []
        for line in lines:
            # Remove lines that look like they're part of the Ollama interface
            if not line.startswith('>>>') and not line.startswith('...') and '[END]' not in line:
...
def generate_basic_response_for_scam_type(message: str, conversation_context: List = [], session_id: str = "") -> str:
    """
    Generate a basic response based on the type of scam detected and conversation context
    """
    # Use session_id to create consistent but varied responses for the same session
    session_hash = hash(session_id) if session_id else 0
    
    # Identify the scam category
    category, score = identify_scam_category(message)
    
    if category != "unknown" and score > 0:
        agent_info = get_agent_info(category)
        responses = agent_info["responses"]
        # Use a combination of message hash, conversation length, and session hash for variety
        idx = (hash(message) + len(conversation_context) + session_hash) % len(responses)
    ...
    idx = (hash(message) + len(conversation_context) + session_hash) % len(default_responses)
...
def generate_agent_response_with_context(text: str, session_id: str, scam_detected: bool,
    ...
    if not scam_detected:
        # More varied normal conversation responses
        normal_responses = [
            "I'm not sure what you mean. Can you explain?",
            "Sorry, I don't understand. Could you rephrase?",
            "Can you provide more details?",
        ...
        idx = (msg_count + hash(text)) % len(normal_responses)
        return normal_responses[idx]

    # Generate response based on the detected scam category with more variety
    return generate_basic_response_for_scam_type(text, conversation_history, session_id)
...
if __name__ == "__main__":
    ...
    for category, data in SCAM_CATEGORIES.items():
        print(f"  * {data['agent_personality']} ({category}): {data['agent_trait']}")
