...
class ConversationStateMachine:
    ...
    def _should_exit_early(self, session: SessionData) -> bool:
        ...
        # If we've extracted good intelligence and are in EXTRACT state
        if session.state == ConversationState.EXTRACT and intel_count >= 1:
            return True
        ...
        # If scammer is asking for sensitive info (time to exit safely)
        last_messages = [msg.text.lower() for msg in session.conversation_history[-3:]]
        for msg in last_messages:
            if any(keyword in msg for keyword in ['pan', 'aadhaar', 'password', 'pin', 'cvv', 'otp']):
                return True
